...
class RequestManager {
  ...
  private async completeRequest(requestId: string) {
    ...
    this.activeRequests.set(requestId, request); // Update status

    try {
      await this.storeFinishedRequest(request);
      // Only remove after successful storage
      this.activeRequests.delete(requestId);
      // Close UI components
      this.closeRequestWindows(requestId);
    } catch (error) {
      console.error('Error completing request:', error);
      this.notifyError('Failed to save request details. The request is completed but data might not be saved.');
    }
  }
